...
# set working folder
setwd("/home/yuq22/Bacteria_TRM/salmonella/multiome_analysis/GRN_on_more_filtered_data/GRN_on_subset")
# load data
#combined <- readRDS("../Res_chip_infection_multiome.rds")
## subset to stem cells to absorptive cell types
#subset_data <- subset(combined, subset = Subtype %in% c("Colon_SC", paste("Colonocyte", seq(4), sep="_")))
#saveRDS(subset_data, file="Res_chip_infection_multiome_s2E.rds")

combined <- readRDS("Res_chip_infection_multiome_s2E.rds")
...
saveRDS(combined, file="Res_chip_infection_with_ATAC_HighResClustering.rds")

#  link peaks to nearby genes based on correlation between gene expression and peak detection rate
# combined <- readRDS("Res_chip_infection_with_ATAC_HighResClustering.rds")
...
saveRDS(combined, file="Res_chip_infection_with_LinkPeaks.rds")
...
saveRDS(combined, file="Res_chip_infection_with_GRN.rds")